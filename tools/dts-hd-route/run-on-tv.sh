...
if [ "${1:-}" != '--apply' ] && [ "${1:-}" != '--session' ]; then
    echo 'DTS-HD controller. Nothing changed.'
...
core_now() {
    ...
    grep -qx 'Codec=DTS_HD' /proc/lgtv-driver/audio/hdmi &&
    grep -qx 'SamplingRate=192000' /proc/lgtv-driver/audio/hdmi &&
    grep -qx 'Connect=HDMI_PORT2' /proc/lgtv-driver/audio/adec.p1
}
core_now || { echo 'Requires live DTS-HD/48kHz on HDMI3; aborting.'; exit 1; }
...
cleanup() {
    ...
    if [ "$modified" = 1 ] && [ -f "$prop" ] &&
       grep -qx 'hdmi-port=(int)2' "$prop" &&
       { core_now || grep -qx 'codec=(int)6' "$prop"; }; then
        restore_input=1
        printf 'drop 1\n' > "$prop" || failed=1
        printf 'start 0\n' > "$prop" || failed=1
    fi
    ...
    if [ "$restore_input" = 1 ]; then
        for name in codec mode sampling-freq channel-num data-drop-mode drop-output-data start drop; do
...
while [ "$i" -lt 10 ]; do
    core_now || exit 1
    if grep -qx 'UserCodec=DTS_HD' /proc/lgtv-driver/audio/adec.p1 &&
...
gain_modified=1
printf 'mute-input true\n' > "$gain_prop"
printf 'mute-output true\n' > "$gain_prop"
printf 'codec 6\n' > "$prop"
printf 'mode 4\n' > "$prop"
printf 'sampling-freq 192000\n' > "$prop"
printf 'channel-num 8\n' > "$prop"
printf 'data-drop-mode 1\n' > "$prop"
printf 'drop-output-data false\n' > "$prop"
printf 'start 1\n' > "$prop"
printf 'drop 0\n' > "$prop"
# Stay muted until the asynchronous callback configures non-PCM transport.
sleep 2
core_now || exit 1
# Exact-build cached argument of audio_hal_earc_set_output_format.
transport=$(devmem 0x264d5bc 32)
printf '%s\n' "$transport" > "$logdir/earc-format-flag.txt"
[ "$transport" = '0x00000001' ] || { echo 'eARC is not non-PCM; aborting muted'; exit 1; }
cat /proc/aaudio/processors/dsp1/output_arc/*/prop > "$logdir/output-before-unmute.prop"
printf 'mute-input false\n' > "$gain_prop"
printf 'mute-output false\n' > "$gain_prop"
if [ "$session" = 1 ]; then echo "DTS-HD supervised session active"; else
    echo "DTS-HD test running for $seconds seconds. Check Q950A audio/format display."
fi
i=0
