...
#include "status_hooks.h"
#include "status_guards.h"
...
static int __init core_init(void)
{
    ...
    for (i = 0; i < ARRAY_SIZE(status_checks); i++)
        if (memcmp((void *)status_checks[i].addr, status_checks[i].bytes, 64)) return -EINVAL;
    ret = bridge_install();
    if (ret) return ret;
    ret = bridge_selftest();
    ...
    ret = register_kprobes(hd_probes, ARRAY_SIZE(hd_probes));
    if (ret) { bridge_remove(); return ret; }
    ret = register_kprobe(&route_probe);
    if (ret) { unregister_kprobes(hd_probes, ARRAY_SIZE(hd_probes)); bridge_remove(); return ret; }
...
{
    if (route_registered) {
        unregister_kprobe(&route_probe);
        unregister_kprobes(hd_probes, ARRAY_SIZE(hd_probes));
    }
    pr_info("dts_hd: status codec7 accepted %d times\n", atomic_read(&hd_status_hits));
    bridge_remove();
