...
source=(R/'tools/dts-hd-route/run-on-tv.sh').read_text()
results=[]
for case in ('success','bad_input','gate_fail','route_fail','missing_graph','source_change','unload_fail','missing_gain','bad_transport'):
 with tempfile.TemporaryDirectory(prefix='dts-controller-') as tmp:
  ...
  (audio/'hdmi').write_text('Codec='+('PCM' if case=='bad_input' else 'DTS_HD')+'\nSamplingRate=192000\nAudioData=1\n')
  ...
  output=proc/'aaudio/processors/dsp1/output_arc/03240001';output.mkdir(parents=True)
  (output/'prop').write_text('stride-shift=(int)4\n')
...
elif n=='devmem':print('0x00000000' if C=='bad_transport' else '0x00000001')
...
'''
  for name in ('id','uname','sha256sum','lsmod','insmod','rmmod','amixer','dmesg','sleep','systemctl','devmem'):
  ...
  if case in ('success','missing_graph','source_change','unload_fail','missing_gain','bad_transport'):assert 'rmmod dts_core_route' in calls
...
(R/'build/reports/hd-controller-tests.json').write_text(json.dumps(results,indent=2)+'\n')
